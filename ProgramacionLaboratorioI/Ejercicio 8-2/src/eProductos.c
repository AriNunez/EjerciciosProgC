...
{
	printf("%5d \t %s \t %d \t %d \t %.2f\n",elemento.idProductos,elemento.descripcion,elemento.nacionalidad,elemento.tipo,elemento.precio);
}
...
int eProductos_MostrarTodos(eProductos vector[],int tam)
{
	...
	{
		puts("\n\t>>Listado Productos");
		printf("%5s\t%5s\t%5s\t%5s\t%5s\n\n","ID","DESCRIPCION","NACIONALIDAD","TIPO","PRECIO"); //TITULOS DE COLUMNAS
		for(i=0;i<tam;i++)
		{
...
int eProductos_Alta(eProductos vector[],int tam)
{
	...
	if(index != -1)
	{
		...
		if(retorno == 1)
		{
			auxiliar.idProductos = eProductos_ObtenerID();
